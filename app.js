...
// 🔐 Система шифрования (E2EE v2.0 — ECDH P-256 + HKDF + AES-256-GCM)
let myIdentity = null;                    // {privateKey: CryptoKey, publicKeyBase64: string} — своя пара ключей
let identityReady = false;                // true когда myIdentity готова к использованию
const peerPublicKeys = new Map();         // username -> publicKeyBase64 (кэш)
const peerPublicKeyObjs = new Map();      // username -> CryptoKey (импортированный публичный ключ)
const directConvKeys = new Map();         // username -> CryptoKey (ECDH-секрет с этим собеседником)
const groupConvKeys = new Map();          // groupId -> CryptoKey (расшифрованный групповой секрет)
const pendingPublicKeyRequests = new Map();  // username -> [{resolve, reject}]
const pendingGroupKeyRequests = new Map();   // groupId -> [{resolve, reject}]
...
// ============================================================================
// 🔐 ФУНКЦИИ ШИФРОВАНИЯ (E2EE v2.0 — ECDH P-256 + HKDF + AES-256-GCM)
// ============================================================================
// Раньше ключ шифрования выводился из собственного пароля отправителя, из-за
// чего получатель НЕ МОГ расшифровать чужие сообщения (у него другой пароль).
// Теперь у каждого пользователя есть постоянная пара ключей ECDH: приватный
// ключ живёт только в IndexedDB этого браузера, публичный публикуется на
// сервере. Ключ переписки — это ECDH shared secret, известный только двум
// участникам разговора (или, для групп, — общий секрет, обёрнутый для
// каждого участника отдельно). Сервер хранит и передаёт только шифротекст
// и публичные/обёрнутые ключи — расшифровать сообщения он не может.

/**
 * Инициализация E2EE-идентичности после входа/регистрации.
 * Загружает пару ключей из IndexedDB или генерирует новую, публикует
 * публичный ключ на сервере (идемпотентно).
 */
async function ensureIdentityKeyPair() {
    if (!window.CryptoUtils || !window.E2EEIdentityStore) {
        console.error('❌ E2EE modules not loaded');
        identityReady = false;
        return false;
    }
    if (!CryptoUtils.isCryptoSupported()) {
        console.error('❌ Web Crypto API не поддерживается в этом браузере');
        identityReady = false;
        return false;
    }

    try {
        let identity = await E2EEIdentityStore.getStoredIdentity(currentUser);

        if (!identity) {
            const generated = await CryptoUtils.generateIdentityKeyPair();
            await E2EEIdentityStore.storeIdentity(currentUser, generated.privateKey, generated.publicKeyBase64);
            identity = { privateKey: generated.privateKey, publicKeyBase64: generated.publicKeyBase64 };
            console.log('🔑 Сгенерирована новая пара ключей E2EE');
        }

        myIdentity = identity;
        identityReady = true;

        // Публикуем публичный ключ на сервере (безопасно вызывать повторно)
        sendToServer({ type: 'publish_public_key', publicKey: identity.publicKeyBase64 });

        return true;
    } catch (error) {
        console.error('❌ ensureIdentityKeyPair error:', error);
        identityReady = false;
        return false;
    }
}
...
/**
 * Очистка ключей E2EE из памяти при выходе из аккаунта
 */
function clearEncryptionState() {
    myIdentity = null;
    identityReady = false;
    peerPublicKeys.clear();
    peerPublicKeyObjs.clear();
    directConvKeys.clear();
    groupConvKeys.clear();
    pendingPublicKeyRequests.clear();
    pendingGroupKeyRequests.clear();
}

/**
 * Запросить публичный ключ собеседника у сервера (с кэшированием)
 * @param {string} username
 * @returns {Promise<CryptoKey|null>}
 */
function requestPeerPublicKey(username) {
    if (peerPublicKeyObjs.has(username)) {
        return Promise.resolve(peerPublicKeyObjs.get(username));
    }
    return new Promise((resolve) => {
        if (!pendingPublicKeyRequests.has(username)) {
            pendingPublicKeyRequests.set(username, []);
            sendToServer({ type: 'get_public_key', targetUsername: username });
        }
        pendingPublicKeyRequests.get(username).push(resolve);
    });
}

/**
 * Обработка ответа сервера с публичным ключом пользователя
 */
async function handlePublicKeyResponse(username, publicKeyBase64) {
    let keyObj = null;
    if (publicKeyBase64) {
        try {
            keyObj = await CryptoUtils.importPeerPublicKey(publicKeyBase64);
            peerPublicKeys.set(username, publicKeyBase64);
            peerPublicKeyObjs.set(username, keyObj);
        } catch (error) {
            console.error('❌ Failed to import peer public key:', error);
        }
    }
    const waiters = pendingPublicKeyRequests.get(username) || [];
    pendingPublicKeyRequests.delete(username);
    waiters.forEach(resolve => resolve(keyObj));
}

/**
 * Получить (или вывести) ключ переписки 1-на-1 с собеседником
 * @param {string} peerUsername
 * @returns {Promise<CryptoKey|null>}
 */
async function getDirectConversationKey(peerUsername) {
    if (!identityReady || !myIdentity || !peerUsername) return null;
    if (directConvKeys.has(peerUsername)) return directConvKeys.get(peerUsername);

    const peerPublicKey = await requestPeerPublicKey(peerUsername);
    if (!peerPublicKey) return null;

    try {
        const key = await CryptoUtils.deriveConversationKey(myIdentity.privateKey, peerPublicKey);
        directConvKeys.set(peerUsername, key);
        return key;
    } catch (error) {
        console.error('❌ getDirectConversationKey error:', error);
        return null;
    }
}

/**
 * Получить (или запросить/развернуть) ключ группового чата
 * @param {string} groupId
 * @returns {Promise<CryptoKey|null>}
 */
async function getGroupConversationKey(groupId) {
    if (!identityReady || !myIdentity || !groupId) return null;
    if (groupConvKeys.has(groupId)) return groupConvKeys.get(groupId);

    return new Promise((resolve) => {
        if (!pendingGroupKeyRequests.has(groupId)) {
            pendingGroupKeyRequests.set(groupId, []);
            sendToServer({ type: 'get_group_key', groupId });
        }
        pendingGroupKeyRequests.get(groupId).push(resolve);
    });
}

/**
 * Обработка ответа сервера с обёрнутым групповым ключом
 */
async function handleGroupKeyResponse(groupId, wrapped) {
    const waiters = pendingGroupKeyRequests.get(groupId) || [];
    pendingGroupKeyRequests.delete(groupId);
    ...
    try {
        const wrapperPublicKey = await requestPeerPublicKey(wrapped.wrappedBy);
        if (!wrapperPublicKey) {
            waiters.forEach(resolve => resolve(null));
            return;
        }
        const conversationKey = await CryptoUtils.deriveConversationKey(myIdentity.privateKey, wrapperPublicKey);
        const groupSecretRaw = await CryptoUtils.unwrapGroupSecretFromPeer(wrapped, conversationKey, groupId);
        const groupKey = await CryptoUtils.importGroupSecretKey(groupSecretRaw);
        groupConvKeys.set(groupId, groupKey);
        waiters.forEach(resolve => resolve(groupKey));
    } catch (error) {
        console.error('❌ handleGroupKeyResponse error:', error);
        waiters.forEach(resolve => resolve(null));
    }
}

/**
 * Создать новый групповой секрет и обернуть его для списка участников
 * (используется при создании группы и при добавлении новых участников)
 * @param {string[]} memberUsernames - Участники, для которых нужно обернуть ключ
 * @returns {Promise<{groupSecretRaw: Uint8Array, shares: Array}|null>}
 */
async function createWrappedGroupKeyShares(groupId, memberUsernames) {
    if (!identityReady || !myIdentity) return null;

    const groupSecretRaw = await CryptoUtils.generateGroupSecret();
    const shares = [];

    for (const member of memberUsernames) {
        let conversationKey;
        if (member === currentUser) {
            // Оборачиваем "для себя", используя собственный публичный ключ —
            // это позволяет нам самим развернуть ключ так же, как это сделают
            // остальные участники (единая логика unwrap для всех).
            const myPublicKeyObj = await CryptoUtils.importPeerPublicKey(myIdentity.publicKeyBase64);
            conversationKey = await CryptoUtils.deriveConversationKey(myIdentity.privateKey, myPublicKeyObj);
        } else {
            const peerPublicKey = await requestPeerPublicKey(member);
            if (!peerPublicKey) {
                console.warn(`⚠️ Нет публичного ключа для ${member}, группа для него не будет зашифрована`);
                continue;
            }
            conversationKey = await CryptoUtils.deriveConversationKey(myIdentity.privateKey, peerPublicKey);
        }

        const wrapped = await CryptoUtils.wrapGroupSecretForPeer(groupSecretRaw, conversationKey, groupId);
        shares.push({ username: member, ...wrapped });
    }

    // Кэшируем свой экземпляр ключа сразу, чтобы не запрашивать его у сервера
    const groupKey = await CryptoUtils.importGroupSecretKey(groupSecretRaw);
    groupConvKeys.set(groupId, groupKey);

    return { groupSecretRaw, shares };
}

/**
 * Обернуть уже существующий групповой ключ для нового участника
 * (вызывается тем, кто добавляет — он уже владеет расшифрованным ключом группы)
 */
async function wrapExistingGroupKeyForNewMember(groupId, newMemberUsername) {
    const groupKey = await getGroupConversationKey(groupId);
    if (!groupKey) return null;

    // Нам нужен именно "сырой" секрет группы, чтобы обернуть его заново под
    // ключ нового участника. Так как groupKey (HKDF-key) неизвлекаем,
    // используем отдельно сохранённый сырой секрет, если он есть в кэше.
    const rawSecret = groupRawSecrets.get(groupId);
    if (!rawSecret) {
        console.error('❌ Нет сырого группового секрета для повторного оборачивания (перелогиньтесь как создатель группы)');
        return null;
    }

    const peerPublicKey = await requestPeerPublicKey(newMemberUsername);
    if (!peerPublicKey) return null;

    const conversationKey = await CryptoUtils.deriveConversationKey(myIdentity.privateKey, peerPublicKey);
    return await CryptoUtils.wrapGroupSecretForPeer(rawSecret, conversationKey, groupId);
}

const groupRawSecrets = new Map(); // groupId -> Uint8Array (только для тех групп, что мы сами создали в этой сессии)

/**
 * Шифрование сообщения перед отправкой
 * @param {string} text - Текст сообщения
 * @param {string} messageId - ID сообщения
 * @param {CryptoKey} conversationKey - Ключ переписки (direct или group)
 * @returns {Promise<{encryptedContent: string, encryptionHint: string}>}
 */
async function encryptOutgoingMessage(text, messageId, conversationKey) {
    if (!conversationKey) {
        throw new Error('Conversation key not available');
    }
    return await CryptoUtils.encryptFullMessage(text, conversationKey, messageId);
}
 ...
 */
async function decryptIncomingMessage(encryptedContent, encryptionHint, messageId, conversationKey) {
    if (!conversationKey) {
        throw new Error('Conversation key not available');
    }
    return await CryptoUtils.decryptFullMessage(encryptedContent, encryptionHint, conversationKey, messageId);
}

/**
 * Пакетная расшифровка истории сообщений личного чата
 * @param {Array} messages - Массив зашифрованных сообщений
 * @param {string} chatName - Имя собеседника
 * @returns {Promise<Array>} - Массив расшифрованных сообщений
 */
async function decryptMessageHistory(messages, chatName) {
    if (!identityReady) return messages;

    const conversationKey = await getDirectConversationKey(chatName);
    if (!conversationKey) return messages;

    return await decryptMessagesWithKey(messages, conversationKey);
}

/**
 * Пакетная расшифровка истории сообщений группового чата
 * @param {Array} messages
 * @param {string} groupId
 * @returns {Promise<Array>}
 */
async function decryptGroupMessageHistory(messages, groupId) {
    if (!identityReady) return messages;

    const conversationKey = await getGroupConversationKey(groupId);
    if (!conversationKey) return messages;

    return await decryptMessagesWithKey(messages, conversationKey);
}

async function decryptMessagesWithKey(messages, conversationKey) {
    const decryptedMessages = [];

    for (const msg of messages) {
        try {
            if (msg.encrypted && msg.encryptedContent && msg.encryptionHint) {
                const messageId = msg.id ? String(msg.id) : msg.timestamp.toString();
        ...
        switch (data.type) {
            case 'register_success':
                ...
                if (data.username && data.token && data.deviceId) {
                    ...
                    // 🔐 Инициализация E2EE (генерация/загрузка пары ключей и публикация публичного ключа)
            ...
            case 'public_key':
                if (data.username) {
                    handlePublicKeyResponse(data.username, data.publicKey || null);
                }
                break;
            case 'public_key_missing':
                if (data.username) {
                    handlePublicKeyResponse(data.username, null);
                }
                break;
            case 'public_keys':
                if (data.keys && typeof data.keys === 'object') {
                    Object.entries(data.keys).forEach(([uname, pk]) => handlePublicKeyResponse(uname, pk));
                    (data.missing || []).forEach(uname => handlePublicKeyResponse(uname, null));
                }
                break;
            case 'group_key':
                if (data.groupId) {
                    handleGroupKeyResponse(data.groupId, {
                        wrappedKey: data.wrappedKey,
                        nonce: data.nonce,
                        salt: data.salt,
                        wrappedBy: data.wrappedBy
                    });
                }
                break;
            case 'group_key_missing':
                if (data.groupId) {
                    handleGroupKeyResponse(data.groupId, null);
                }
                break;
            case 'chat_deleted':
                if (data.chatName) {
            ...
            case 'group_created':
                if (data.group && data.group.id) {
                    ...
                    renderGroups();
                    // 🔐 E2EE: если это мы только что создали группу — генерируем
                    // и рассылаем групповой ключ теперь, когда известен groupId
                    setupGroupEncryptionAfterCreation(data.group.id);
                }
                break;
...
async function handleLoginSuccess(data) {
    ...
    // 🔧 FIX: Обновляем sidebar компонент с данными пользователя
...
// ============================================================================
async function loadMessageHistory(messages, chatName, groupId) {
    if (!messages || !Array.isArray(messages)) return;

    console.log(`📜 Loaded ${messages.length} messages for ${chatName || groupId}`);

    // 🔐 Расшифровываем историю, если она зашифрована
    try {
        if (groupId) {
            messages = await decryptGroupMessageHistory(messages, groupId);
        } else if (chatName && chatName !== 'general') {
            messages = await decryptMessageHistory(messages, chatName);
        }
    } catch (error) {
...
async function handleMessageReceive(data) {
    ...
    if (data.isEncrypted && data.encryptedContent && data.encryptionHint && identityReady) {
        try {
            // Собеседник в этой переписке: если сообщение от нас самих (эхо
            // подтверждения отправки) — это тот, кому мы писали (privateTo),
            // иначе — тот, кто нам написал (sender).
            const peerUsername = data.sender === currentUser ? (data.privateTo || selectedUser) : data.sender;
            const conversationKey = await getDirectConversationKey(peerUsername);
...
function updateFooterProfile() {
    ...
    // Проверяем аватар из профиля - 🔒 БЕЗОПАСНОЕ создание
    try {
...
function renderGroupMembersSelect() {
    ...
    if (fragment.children.length === 0) {
        ...
        textSpan.textContent = 'Нет доступных пользователей';
        noResultsDiv.appendChild(textSpan);

        const smallEl = document.createElement('small');
        smallEl.textContent = 'Показываются только пользователи с которыми есть активный чат';
        noResultsDiv.appendChild(smallEl);

        DOM.groupMembersSelect.innerHTML = '';
        DOM.groupMembersSelect.appendChild(noResultsDiv);
    }
...
async function createGroup() {
    ...
    // Отправляем запрос на сервер и ждём подтверждения с реальным groupId
    // (он генерируется сервером, поэтому шифрование группового ключа
    // происходит вторым шагом — см. обработку 'group_created')
    pendingGroupCreation = { members: [...selectedMembers, currentUser] };
...
let pendingGroupCreation = null; // {members: string[]} — участники группы, которую мы только что создали

/**
 * 🔐 E2EE: после того как сервер подтвердил создание группы и сообщил её ID,
 * генерируем групповой секрет и рассылаем обёрнутые копии ключа всем
 * участникам (только создатель группы это делает).
 */
async function setupGroupEncryptionAfterCreation(groupId) {
    if (!pendingGroupCreation || !identityReady) {
        pendingGroupCreation = null;
        return;
    }

    const members = pendingGroupCreation.members;
    pendingGroupCreation = null;

    try {
        const result = await createWrappedGroupKeyShares(groupId, members);
        if (!result) return;

        groupRawSecrets.set(groupId, result.groupSecretRaw);

        if (result.shares.length > 0) {
            sendToServer({
                type: 'set_group_key_shares',
                groupId,
                memberKeys: result.shares
            });
        }
    } catch (error) {
        console.error('❌ Failed to set up group encryption:', error);
    }
}
...
async function sendMessage() {
    ...
    // 🔐 Шифрование сообщения если это личный чат или группа (не общий чат)
    // и ключ переписки удалось получить/вывести
    if (identityReady && (selectedGroup || selectedUser)) {
        try {
            const conversationKey = selectedGroup
                ? await getGroupConversationKey(selectedGroup)
                : await getDirectConversationKey(selectedUser);

            if (conversationKey) {
                const messageId = CryptoUtils.generateMessageId();
                const encrypted = await encryptOutgoingMessage(text, messageId, conversationKey);
                encryptedContent = encrypted.encryptedContent;
                encryptionHint = encrypted.encryptionHint;
                messageText = '[🔒 Зашифровано]'; // Заменяем текст для отображения
            } else {
                console.warn('⚠️ Не удалось получить ключ переписки, сообщение отправлено без шифрования');
            }
        } catch (error) {
...
function createMessageElement(data, isOwn = false) {
    ...
    if (data.files && data.files.length > 0) {
        ...
        data.files.forEach(fileData => {
            const fileElement = createFileHtml(fileData);
            if (fileElement) {
...
function showReplyIndicator() {
    ...
    replyTextSpan.innerHTML = 'Ответ на сообщение: <strong></strong>';
    const strongEl = replyTextSpan.querySelector('strong');
    if (strongEl) {
        strongEl.textContent = escapeHtml(replyToMessage.sender || '');
    }
    indicator.appendChild(replyTextSpan);

    const cancelBtn = document.createElement('button');
    cancelBtn.className = 'reply-cancel';
    cancelBtn.type = 'button';
    cancelBtn.title = 'Отменить ответ';
...
/**
 * Создание DOM-элемента для файла в сообщении
 * 🔒 БЕЗОПАСНОСТЬ: валидация всех полей перед использованием
 * @param {Object} fileData - Данные файла
 * @returns {HTMLElement|null} - DOM-элемент файла
 */
function createFileHtml(fileData) {
    // 🔒 Валидация входных данных
    if (!fileData || typeof fileData !== 'object') {
        console.warn('⚠️ createFileHtml: invalid fileData');
        return null;
    }
    ...
    if (typeof type !== 'string' || typeof name !== 'string' || typeof data !== 'string') {
        console.warn('⚠️ createFileHtml: invalid field types');
        return null;
    }

    // 🔒 Проверка name на XSS
    if (/[<>\"'&]/.test(name)) {
        console.warn('⚠️ createFileHtml: potential XSS in filename');
        return null;
    }

    // 🔒 Проверка data URI scheme
    if (!data.startsWith('data:') && !data.startsWith('blob:')) {
        console.warn('⚠️ createFileHtml: invalid data URI');
        return null;
    }
 ...
 * 🔒 БЕЗОПАСНОСТЬ: санитизация всех данных
 * @param {string} avatarUrl - URL аватарки
 */
function updateAvatarDisplay(avatarUrl) {
    // 🔒 Валидация currentUser
    if (!currentUser || typeof currentUser !== 'string') {
        console.warn('⚠️ updateAvatarDisplay: invalid currentUser');
        return;
    }
    ...
    // В footer sidebar - 🔒 БЕЗОПАСНОЕ создание без innerHTML
    if (DOM.footerUserAvatar) {
        // Очищаем контейнер
        DOM.footerUserAvatar.innerHTML = '';

        if (avatarUrl) {
            // Создаём img элемент безопасно
            const img = document.createElement('img');
            img.src = url;
            img.alt = 'Аватар';
            DOM.footerUserAvatar.appendChild(img);
        } else {
            // Создаём инициалы безопасно
            const initialsSpan = document.createElement('span');
            initialsSpan.className = 'avatar-initials';
            // 🔒 Безопасное получение инициалов
            initialsSpan.textContent = currentUser.slice(0, 2).toUpperCase();
...
function openProfile(userId) {
    ...
    // 🔧 FIX: Защита от XSS - используем textContent вместо innerHTML
    const name = escapeHtml(userId);
    ...
    if (DOM.profileUserName) {
        // 🔧 FIX: Защита от XSS - textContent автоматически экранирует
...
function performLogout() {
    ...
    // 🔐 Очищаем ключи шифрования (E2EE-идентичность в IndexedDB не удаляем —
    // она понадобится при следующем входе с этого устройства, чтобы читать
    // старую переписку)
...
function loadSettings() {
    try {
        ...
        if (!settings) {
            // Применяем значения по умолчанию при первом запуске
            // 🔧 FIX: Цвет сообщений по умолчанию (#7B2CBF)
            ...
            // 🔧 FIX: Тема по умолчанию (тёмная)
            document.documentElement.setAttribute('data-theme', 'dark');
            if (DOM.themeSelect) DOM.themeSelect.value = 'dark';

            // 🔧 FIX: Акцентный цвет по умолчанию (#7B2CBF)
            document.documentElement.style.setProperty('--accent', '#7B2CBF');
            document.documentElement.style.setProperty('--accent-hover', '#9D4EDD');
            if (DOM.accentColorSelect) DOM.accentColorSelect.value = '#7B2CBF';

            // 🔧 FIX: Размер шрифта по умолчанию (средний)
            document.body.classList.add('font-medium');
            if (DOM.fontSizeSelect) DOM.fontSizeSelect.value = '14';
            ...
            // 🔧 FIX: Звук уведомлений по умолчанию включён
            soundEnabled = true;
            if (DOM.soundNotify) DOM.soundNotify.checked = true;
...
function handleTwoFAMessage(data) {
    ...
    switch (data.type) {
        ...
        case '2fa_enabled':
            ...
            // Показываем резервные коды - 🔒 БЕЗОПАСНОСТЬ
            const codesContainer = document.getElementById('twoFactorBackupCodes');
            if (codesContainer && Array.isArray(data.backupCodes)) {
...
function showLogin2FAForm() {
    ...
    h3.textContent = '🔐 Двухфакторная аутентификация';
    twoFactorForm.appendChild(h3);

    const textP = document.createElement('p');
    textP.className = 'two-factor-login-text';
    textP.textContent = 'Введите код из приложения аутентификации';
    twoFactorForm.appendChild(textP);

    const formGroup1 = document.createElement('div');
    formGroup1.className = 'form-group';

    const label1 = document.createElement('label');
    label1.htmlFor = 'login2FACodeInput';
    label1.textContent = 'Код';
    formGroup1.appendChild(label1);

    const codeInput = document.createElement('input');
    codeInput.type = 'text';
    codeInput.id = 'login2FACodeInput';
    codeInput.placeholder = '123456';
    codeInput.maxLength = 6;
    codeInput.pattern = '[0-9]*';
    codeInput.inputMode = 'numeric';
    formGroup1.appendChild(codeInput);

    twoFactorForm.appendChild(formGroup1);
    ...
    const checkbox = document.createElement('input');
    checkbox.type = 'checkbox';
    checkbox.id = 'loginUseBackupCodeCheckbox';
    label2.appendChild(checkbox);
    label2.appendChild(document.createTextNode(' Использовать резервный код'));
    formGroup2.appendChild(label2);

    twoFactorForm.appendChild(formGroup2);

    const submitBtn = document.createElement('button');
    submitBtn.id = 'login2FASubmitBtn';
    submitBtn.className = 'btn-primary';
    submitBtn.type = 'button';
    submitBtn.textContent = 'Войти';
    twoFactorForm.appendChild(submitBtn);

    const cancelBtn = document.createElement('button');
    cancelBtn.id = 'login2FACancelBtn';
    cancelBtn.className = 'btn-secondary';
    cancelBtn.type = 'button';
    cancelBtn.textContent = 'Отмена';
    twoFactorForm.appendChild(cancelBtn);

    const statusDiv = document.createElement('div');
    statusDiv.id = 'login2FAStatus';
    statusDiv.className = 'status-message';
    statusDiv.setAttribute('role', 'alert');
    statusDiv.setAttribute('aria-live', 'polite');
    twoFactorForm.appendChild(statusDiv);
